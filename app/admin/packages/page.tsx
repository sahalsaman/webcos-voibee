...
export default async function AdminPackagesPage() {
  const packages = (await listAdminTrips()) as TripDTO[];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{packages.length} total</p>
        </div>
        <Button asChild variant="gradient">
          <Link href="/admin/packages/new"><Plus className="size-4" /> New Package</Link>
        </Button>
      </div>
      ...
      {packages.length ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-left text-muted-foreground">
                  <th className="p-4 font-medium">Packages</th>
              ...
              <tbody>
                {packages.map((t) => {
                  ...
                  const duration = t.packageOptions?.[0]?.label;
                  return (
                    ...
                    <td className="p-4"><p className="font-medium">{formatINR(t.basePrice)}</p><p className="text-xs text-muted-foreground">{duration || "Single price"}</p></td>
        ...
        <EmptyState
          ...
          description="Create your first holiday package to start selling."
          action={
            <Button asChild variant="gradient">
              <Link href="/admin/packages/new"><Plus className="size-4" /> New Package</Link>
            </Button>
